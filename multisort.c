...
unsigned bots_arg_size, bots_app_cutoff_value, bots_app_cutoff_value_1, bots_app_cutoff_value_2;
...
//void cilksort_par(ELM *low, ELM *tmp, long size)
  VOID_TASK_3( cilksort_par , ELM*, low , ELM*, tmp, long , size)
{
     /*
...
static void *alloc_dist( size_t size )
{
  #if defined(__TILECC__)
    alloc_attr_t attr = ALLOC_INIT;

    alloc_set_home( &attr, ALLOC_HOME_HASH );
    return alloc_map( &attr, size );
...
LOOP_BODY_1( fill_and_scramble_row, LARGE_BODY, unsigned, row, unsigned, cols )
{
  unsigned seed = row*2;
  int i,lim = (row+1)*cols,base = row*cols;
  ...
  for( i = base; i < lim; i++ ) {
    ELM tmp = array[i];
    unsigned j = base + rand_r( &seed ) % cols;
    array[i] = array[j];
    array[j] = tmp;
  }
} 

LOOP_BODY_2( scramble_column, LARGE_BODY, unsigned, col, unsigned, cols, unsigned, rows )
{
  unsigned seed = col*2+1;
  int i;

  for( i = 0; i < rows; i++ ) {
    ELM tmp = array[i*cols+col];
    unsigned j = rand_r( &seed ) % rows;
    array[i*cols+col] = array[j*cols+col];
    array[j*cols+col] = tmp;
  }
} 

LOOP_BODY_2( copy_element, SMALL_BODY, unsigned, i, ELM *, dst, ELM *, src )
{
  dst[i] = src[i];
}
...
VOID_TASK_1( sort_init, int, size )
{
    const int elems_per_line = 16; // Magic!
    unsigned long rows,cols,i;

    i = size / elems_per_line;
    rows = 1;
    while( rows * rows < i ) rows++;

    if( rows*rows != i ) {
      fprintf( stderr, "Bad size %d\n", size );
    }

    cols = (i/rows) * elems_per_line;
    
    array = (ELM *) alloc_dist(size * sizeof(ELM));
    tmp   = (ELM *) alloc_dist(size * sizeof(ELM));
    saved = (ELM *) alloc_dist(size * sizeof(ELM));

    FOR( fill_and_scramble_row, 0, rows, cols );
    FOR( scramble_column, 0, cols, cols, rows ); 
    FOR( copy_element, 0, size, saved, array );   
}


static void sort_verify ( void )
{
     int i, success = 1;
     for (i = 0; i < bots_arg_size; ++i)
	  if (array[i] != i)
...
TASK_2( int, main, int, argc, char **, argv )
{
   ...
   if( argc < 6 ) {
      fprintf( stderr, "Usage: multisort <woolopt>... <merge cutoff> <qs cutoff> <insertion cutoff> <size> <reps>\n" );
      exit( 2 );
   }

   bots_app_cutoff_value   = atoi( argv[ 1 ] );
   bots_app_cutoff_value_1 = atoi( argv[ 2 ] );
   bots_app_cutoff_value_2 = atoi( argv[ 3 ] );

   bots_arg_size = atoi( argv[ 4 ] );
   n = atoi( argv[ 5 ] );

   printf( "Initializing ...\n" );
   CALL( sort_init, bots_arg_size );
   printf( "Sorting ... \n" );
   for( i = 0; i < n; i++ ) {
     CALL( cilksort_par,array, tmp, bots_arg_size);
     FOR( copy_element, 0, bots_arg_size, array, saved );
   }
   CALL( cilksort_par,array, tmp, bots_arg_size);
   printf( "Verifying ...\n" );
   sort_verify();
